...
import {
  HistorialSesionesService, SesionHistorial, HistorialFiltros,
  ComparacionSesion, JuegoOpcion
...
<div class="hs-root">
  ...
  <div class="hs-header">
    <div class="hs-header-left">
      ...
      <div>
        ...
        <p class="hs-sub">{{ nombrePerfil ? nombrePerfil : 'Historial completo por juego y nivel' }}</p>
      </div>
    </div>
              ...
              @if (cargandoComparacion) {
                ...
                <div class="comparacion-wrap">
                  ...
                  @if (!comparacion || !comparacion.haySesionAnterior) {
                    <p class="comp-empty">Esta es la primera sesión registrada de este juego para este perfil.</p>
                  } @else {
                    <div class="comp-grid">
                      <div class="comp-item">
                        ...
                        <div class="comp-vals">
                          ...
                          <span class="comp-delta" [class.up]="(comparacion.deltaPorcentajeAciertos ?? 0) > 0"
                      ...
                      <div class="comp-item">
                        ...
                        <div class="comp-vals">
                          <span>{{ formatMs(comparacion.tiempoRespuestaPromedioMsAnterior) }}</span>
                          <span class="comp-arrow">→</span>
                          <span>{{ formatMs(comparacion.tiempoRespuestaPromedioMsActual) }}</span>
                          <span class="comp-delta" [class.up]="(comparacion.deltaTiempoRespuestaPromedioMs ?? 0) < 0"
                      ...
                      <div class="comp-item">
                        ...
                        <div class="comp-vals">
                          ...
                          <span class="comp-delta" [class.up]="(comparacion.deltaPuntaje ?? 0) > 0"
                      ...
                      <div class="comp-item">
                        ...
                        <div class="comp-vals">
                          <span>{{ formatDuracion(comparacion.duracionSegundosAnterior) }}</span>
                          <span class="comp-arrow">→</span>
  ...
  styles: [`
    *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
    .hs-root { font-family:'Inter',-apple-system,sans-serif; color:#1E293B;
               display:flex; flex-direction:column; gap:18px; padding:24px 28px 48px;
               max-width:1100px; margin:0 auto; }

    /* ── Cabecera ── */
    .hs-header { display:flex; align-items:flex-start; justify-content:space-between; gap:16px; flex-wrap:wrap; }
    .hs-header-left { display:flex; align-items:flex-start; gap:14px; }
    .btn-back { background:#F3F0FF; color:#5B21B6; border:none; border-radius:10px; padding:8px 14px;
                font-size:13px; font-weight:700; cursor:pointer; font-family:inherit; white-space:nowrap; margin-top:2px; }
    .btn-back:hover { background:#EDE9FE; }
    .hs-title { font-size:20px; font-weight:900; color:#1E1B4B; }
    .hs-sub   { font-size:12.5px; color:#94A3B8; margin-top:4px; }
    ...
    /* ── Filtros ── */
    .filtros-card { background:white; border-radius:16px; padding:18px 20px; box-shadow:0 2px 10px rgba(91,33,182,.06); }
    .filtros-row { display:flex; flex-wrap:wrap; gap:12px; align-items:flex-end; }
    .filtro-group { display:flex; flex-direction:column; gap:5px; min-width:150px; }
    ...
    .filtro-input { padding:9px 12px; border:2px solid #E4DEFF; border-radius:10px; font-size:13px;
                    font-family:inherit; color:#1E293B; outline:none; background:white; }
    ...
    .empty-state { display:flex; flex-direction:column; align-items:center; gap:12px; padding:48px;
                   background:white; border-radius:16px; box-shadow:0 2px 10px rgba(91,33,182,.06); }
    .empty-ico { font-size:48px; }
    .empty-state p { color:#64748B; font-size:14px; }
    .btn-limpiar-empty { background:#EDE9FE; color:#5B21B6; border:none; border-radius:10px; padding:8px 20px; font-size:13px; font-weight:700; cursor:pointer; }

    /* ── Lista de sesiones ── */
    .sesiones-list { display:flex; flex-direction:column; gap:10px; }
    .sesion-card { background:white; border-radius:16px; box-shadow:0 2px 10px rgba(91,33,182,.06); overflow:hidden;
                   border:2px solid transparent; transition:border-color .15s; }
    .sesion-card.expandida { border-color:#C4B5FD; }
    .sesion-row { display:flex; align-items:center; gap:16px; padding:14px 18px; cursor:pointer; }
    .sesion-row:hover { background:#FAFAFE; }
    .sr-fecha { min-width:150px; }
    .sr-fecha-dia { font-size:12.5px; font-weight:700; color:#334155; }
    .sr-juego { font-size:11.5px; color:#7C3AED; font-weight:700; margin-top:2px; }
    .sr-nivel { min-width:80px; }
    .nivel-badge { display:inline-block; padding:3px 10px; border-radius:20px; font-size:10.5px; font-weight:800; }
    .nivel-facil   { background:#DCFCE7; color:#166534; }
    .nivel-medio   { background:#FEF9C3; color:#854D0E; }
    .nivel-dificil { background:#FEE2E2; color:#991B1B; }
    .nivel-experto { background:#EDE9FE; color:#5B21B6; }
    .sr-metric { min-width:90px; text-align:center; }
    .sr-metric-val { font-size:14px; font-weight:800; color:#1E1B4B; }
    .sr-metric-lbl { font-size:10px; color:#94A3B8; margin-top:2px; }
    .sr-estado { font-size:11px; font-weight:800; padding:4px 10px; border-radius:20px; flex-shrink:0; white-space:nowrap; margin-left:auto; }
    .sr-estado.ok  { background:#F0FDF4; color:#16A34A; }
    .sr-estado.inc { background:#F8FAFC; color:#94A3B8; }
    .sr-chevron { font-size:11px; color:#94A3B8; flex-shrink:0; width:16px; text-align:center; }

    /* ── Detalle expandido (CA-04) ── */
    .sesion-detalle { padding:0 18px 18px; border-top:1px solid #F1F5F9; }
    .detalle-loading { display:flex; justify-content:center; padding:20px; }
    .detalle-grid { display:flex; gap:12px; margin-top:14px; flex-wrap:wrap; }
    .detalle-chip { background:#F8F7FF; border-radius:12px; padding:10px 16px; min-width:160px; }
    .detalle-chip.chip-warn { background:#FFFBEB; }
    .dc-lbl { font-size:10.5px; color:#94A3B8; font-weight:700; text-transform:uppercase; letter-spacing:.4px; }
    .dc-val { font-size:15px; font-weight:800; color:#1E1B4B; margin-top:3px; }
    .comparacion-wrap { margin-top:16px; }
    .comp-title { font-size:12.5px; font-weight:800; color:#1E1B4B; margin-bottom:10px; }
    .comp-empty { font-size:12.5px; color:#94A3B8; }
    .comp-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(220px,1fr)); gap:10px; }
    .comp-item { background:#F8F7FF; border-radius:12px; padding:10px 14px; }
    .comp-lbl { font-size:10.5px; color:#94A3B8; font-weight:700; margin-bottom:5px; }
    .comp-vals { display:flex; align-items:center; gap:6px; font-size:12.5px; font-weight:700; color:#334155; flex-wrap:wrap; }
    .comp-arrow { color:#CBD5E1; }
    .comp-delta { font-size:11px; font-weight:800; padding:2px 7px; border-radius:20px; background:#F1F5F9; color:#64748B; }
    .comp-delta.up   { background:#DCFCE7; color:#16A34A; }
    .comp-delta.down { background:#FEE2E2; color:#DC2626; }

    /* ── Paginación ── */
    .paginacion { display:flex; justify-content:center; align-items:center; gap:6px; flex-wrap:wrap; }
    .pag-btn { width:36px; height:36px; border:2px solid #E4DEFF; border-radius:10px; background:white; color:#5B21B6;
...
export class HistorialSesionesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.historialService.getJuegos().pipe(catchError(() => of([]))).subscribe(j => {
      this.juegos = j;
  ...
  toggleExpandir(s: SesionHistorial): void {
    ...
    this.historialService.obtenerComparacion(this.perfilId, s.id)
      .pipe(catchError(() => of(null)))
      .subscribe(comp => {
